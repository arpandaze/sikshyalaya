...
const Note = ({ noteTime, title, tags, content, state, onClose, onSave, onDelete, ...rest }) => {
				...
				<Grid container direction="row" alignItems="center">
					<Grid item xs={11} className="note_titleTextContainer">
						...
						<Grid container direction="column" alignItems="flex-end">
							<Grid item xs={6} className="note_timeContainer">
			...
			<Grid item className="note_notePadBot">
				<Grid container direction="column">
					<Grid item className="note_TagCreatorContainer">
						<Grid container direction="row" alignItems="center" className="note_TagCreatorInside">
							<Grid item className="note_tagIconContainer">
								<IconProvider addStyles="note_TagIcon" iconType="tags" />
							</Grid>
							<Grid item>
								<Grid container direction="row" className="note_tagListContainer">
									{stateTag &&
										stateTag.map((tagValue, index) => (
											<Grid item className="note_tag" key={index}>
												<Tag key={index} tagName={tagValue} onDelete={handleRemoveTag} />
											</Grid>
										))}
								</Grid>
							</Grid>
							...
							<Grid item>
								<Button
									name="+"
									addStyles={tagClass ? "note_tagPlusButton" : "note_tagErrorButton"}
									onClicked={() => (isTagCreator ? handleSubmitTag() : handleCreateTag())}
								/>
							</Grid>
						</Grid>
					</Grid>
