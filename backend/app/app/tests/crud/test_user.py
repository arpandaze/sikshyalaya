...
def test_check_if_user_is_active(db: Session) -> None:
    ...
    user_in = UserCreate(email=email, password=password)
    user = crud.crud_user.create(db, obj_in=user_in)
    is_active = crud.crud_user.is_active(user)
...
def test_check_if_user_is_superuser(db: Session) -> None:
    ...
    user = crud.crud_user.create(db, obj_in=user_in)
    is_superuser = crud.crud_user.is_superuser(user)
...
def test_get_user(db: Session) -> None:
    ...
    user = crud.crud_user.create(db, obj_in=user_in)
    user_2 = crud.crud_user.get(db, id=user.id)
    assert user_2
...
def test_update_user(db: Session) -> None:
    ...
    crud.crud_user.update(db, db_obj=user, obj_in=user_in_update)
    user_2 = crud.crud_user.get(db, id=user.id)
    assert user_2
    assert user.email == user_2.email
